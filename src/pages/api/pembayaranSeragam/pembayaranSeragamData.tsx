...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const hasNextPage = getPembayaranSeragam.length > pageSize
      const data = hasNextPage ? getPembayaranSeragam.slice(0, pageSize) : getPembayaranSeragam
      res.status(200).json({ message: 'Get Data successful', getPembayaranSeragam: data, hasNextPage })
    } catch (error) {
